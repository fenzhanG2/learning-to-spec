...
Contextual model findings are applied only within their supplied source field and byte-identical duplicate fields. A matching phrase in a different surrounding context is not automatically added: it may be a technical fixture, a quotation, or a different speaker. Such a field needs its own model finding or user-selected redaction. This narrows model-discovered transformations without weakening local hard-secret removal or changing previously selected local identifier occurrences.
