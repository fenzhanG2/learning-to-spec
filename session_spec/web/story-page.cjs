...
<details class="proof"><summary>${translate('想核对结论？展开本次验证的范围')}</summary><div class="table-scroll" role="region" aria-label="${translate('本次验证范围')}" tabindex="0"><table><thead><tr><th>${translate('要回答的问题')}</th><th>${translate('实际看到的')}</th><th>${translate('还不能据此断言')}</th></tr></thead><tbody>${checks}</tbody></table></div><p>${translate('这里只汇总会话中已有的观察，不代表今天重新执行过这些检查。完整依据随交接文档保存在 _support 目录。')}</p></details>
