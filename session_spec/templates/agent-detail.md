...
## Source-bound modality and coverage

Across the whole edition, keep requested, required, permitted, forbidden, out-of-goal, proposed, observed, reported and unknown distinct. Preserve the source speaker, action, object, time and condition. Not performed does not mean forbidden. A non-goal does not establish a prohibition. A prerequisite for a possible future action neither requires that action now nor prohibits it absolutely. Preserve genuine prohibitions at their stated scope; later permission changes only what it actually authorizes. Do not turn missing evidence into permission, a ban or a new mandatory task.

Unspecified test inputs establish neither coverage nor noncoverage. Test names and passing counts cannot prove that an unnamed input class was never exercised. A negative claim needs corresponding scoped source evidence; otherwise say coverage is not established in this record. Distinguish an explicitly reported absence from independently observed absence. Preserve whether checks were added during the same change and what their actual inputs/assertions establish; do not invent test bodies or independent validation. Apply these distinctions to short labels, stop conditions and reuse advice as well as the main explanation.

Titles and decks require the same evidence strength as the body. Diagnosis is not implementation. A qualified body cannot repair a headline that asserts an unrecorded change or success. Current nonoccurrence is not impossibility or proof of dead logic. Do not justify removing a branch merely because current inputs do not trigger it; establish the governing input domain, intended behavior and relevant implementation. A filename listing is not a content inspection. Preserve what was actually listed, read or searched and its scope; uninspected contents cannot support a project-wide absence claim.

Acceptance must distinguish the reported failure from success across continuation and recipes. An exception must be supported by the governing source contract, not introduced to excuse the very failure being checked. Unresolved representation choices remain proposed, not user requirements. Do not invent a mandatory output policy to settle them; retain the concrete uncertainty and any necessary decision before a consequential change.

Questioning a proposition does not establish belief in it or its disproof. Types and identifiers do not establish actual values or historical beliefs. Do not invent failed hypotheses, overturned assumptions or turning points to dramatize the story, title or closing. When the source only asks or clarifies, narrate asking or clarification. A transferable lesson must remain conditional and grounded in the recorded mechanism and evidence, not an invented historical belief. Apply the same authority and coverage limits throughout the edition, including its opening and closing.
...
Preserve consequential control flow in source order: guards, scoped returns/throws, and applicable cleanup or exception paths. Trace identifiers through recorded assignments and arguments; matching names do not establish shared origin. Do not describe a later operation as reached when an earlier exit prevents it.

For multiple optional controls, keep each default scoped to its own control. Distinguish "an unset filter imposes no restriction of its own" from "either unset filter bypasses the other configured restriction"; the latter requires explicit source support. Check a mixed case (one absent, one configured and failing) before writing an overall allow/deny claim. A field's plain string type or identifier-like name does not establish UUID/email/opaque semantics: attribute a historical format guess to its speaker, and preserve a later explicit correction. A recorded user clarification defines the intended contract; do not invent a mandatory real-payload/log-access prerequisite before ordinary offline implementation unless the source requires it. Label optional drift checks as optional, while preserving genuinely necessary state/permission checks. Repository-wide absence claims require repository-wide evidence; otherwise name the inspected files or the recorded search scope.
...
Review the composed Agent document, not each field in isolation. Base Markdown owns the working contract and mechanism; resume owns the current checkpoint and first move; continuation owns the executable conditional route; history owns the actual failure/correction and state change; recipes and paths add transfer conditions rather than another retelling. Do not recreate structured sections under alternate headings in base Markdown. A brief reminder at a consequential action can be necessary; preserve its conditions, commands, parameters, uncertainty and failed-route evidence. Compress repeated explanation, not required fields or technical meaning. Shared facts across independently readable Human and Agent documents are not within-document redundancy.

Input accounting is not a request to narrate every input. Keep every human_input mapped, but attach an unrelated greeting or aside to the nearest substantive phase's human_refs without creating a warm-up phase, expanding its summary, or adding it to resume or the working contract. Explain that non-task treatment in the private human_input_coverage ledger. Aggregate phase refs should support the actual technical claims, not manufacture a causal link from small talk. Do not announce omitted asides to either reader. A short correction, confirmation that selects an option, failed attempt, changed requirement, authorization limit or acceptance condition is material even if awkward or embarrassing; preserve its consequence. When relevance is uncertain, keep the potentially material condition with its uncertainty rather than silently discard it. This is editorial grouping, not privacy removal: the source and evidence remain unchanged unless the user selected reduction separately.

Omit irrelevant non-task asides without narrating their presence or omission. Do not replace the aside with a visible announcement that conversation was filtered or unrelated material existed. Keep internal source accounting; do not apply this omission rule to technical failures, corrections, constraints or unresolved acceptance.

A recorded rationale must have a short literal source span expressing the choice reason, at that phase or earlier; it is checked separately during source review. A failed test or successful edit alone records no speaker's motive. An explanation only supplied later belongs at that later point or is explicitly retrospective/inferred, not a statement of what the actor understood earlier. Do not add quote fields or raw reasoning to the output record. Prefer not_recorded with empty text/refs if the tool finding already says everything useful.
...
Tool names and source refs can appear inline; raw arguments/results are not automatically rendered or executed. Full payloads, including missing/ambiguous results, remain in private export internals, not recipient prerequisites. In finding retain the meaningful observation, not a log excerpt by default. A rerun marker, empty output, result_recorded or tool success alone is not task completion; assess acceptance against the actual commission rather than requiring excluded integration or release. Preserve upstream truncation/missing-image limits. Never reconstruct hidden chain-of-thought.
