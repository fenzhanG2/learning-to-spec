...
When several optional controls interact, inspect a mixed case: one control absent, another enabled but rejecting. A per-control default must not silently become an overall bypass. Verify the governing contract rather than inventing AND/OR semantics. Also distinguish an agent's historical format guess from the declared type and later user correction; plain string types do not prove a UUID format. Question mandatory runtime/private-log prerequisites not required by the source, and limit absence claims to the actually inspected scope.
For a consequential predicate on a collection, check whether any/all, absent/empty, mixed-match inputs and rejection side effects are actually specified. If the package cannot settle a necessary choice, request a contract lookup or clarification before implementation instead of supplying your preferred semantics. An invented edge-case requirement is not a source fact; do not treat a hypothetical test as observed evidence. Check this only when material to the recorded task.
