...
The Human preview is a display-only projection: renderer styles remain visible, but its empty iframe sandbox and preview-specific policy disable scripts, forms and network content. Non-fragment links are disabled with guidance to the Open buttons above; within-document navigation remains available. This projection never changes the saved HTML, Markdown, ZIP or their hashes.
